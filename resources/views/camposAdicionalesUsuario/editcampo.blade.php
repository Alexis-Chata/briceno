...
@section('content_header')
    ...
    <form action="/camposAdicionales/{{$campos->id}}/update2" method="POST">
        @csrf
        <div class="mb-3">
            <label for="" class="form-label">Nombre Corto</label>
            <input id="shortname" name="shortname" type="text" class="form-control" tabindex="9" value="{{$campos->shortname}}">
            @error('shortname')
            <small>*{{$message}}</small>
        @enderror
        </div>
        <div class="mb-3">
            ...
            <input id="name" name="name" type="text" class="form-control" tabindex="9" value="{{$campos->name}}">
            @error('name')
            <small>*{{$message}}</small>
        @enderror
        </div>
        <div class="form-floating mb-3">
            ...
            <textarea class="form-control" placeholder="escriba aqui" name='descripcion' id="descripcion" >{{$campos->descripcion}}</textarea>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-select form-control" name='alumno_info_category_id' id="alumno_info_category_id">
                ...
                @if ($categorias->id == $campos->alumno_info_category_id)
        ...
        <div class="mb-3">
            ...
            <input id="defaultdata" name="defaultdata" type="text" class="form-control" tabindex="9" value="{{$campos->defaultdata}}">
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Mostrar tamaño</label>
            <input id="mtamaño" name="mtamaño" type="text" class="form-control" tabindex="9" value="{{$campos->param1}}">
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Longitud máxima</label>
            <input id="ltexto" name="ltexto" type="text" class="form-control" tabindex="9" value="{{$campos->param2}}">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-select form-control" name='enlace' id="enlace">
                <option value="0">No</option>
                <option value="1">Si</option>
            </select>
        </div>
